...
- **parameters.ini** : Files that store hyperparameter values for model training<br>
- **main.py** : Study the model with the TF-IDF feature of each document in *Input_Data* folder, evaluate the model with the TF-IDF feature of the data in the *Val_Data* folder, and output the result value with the TF-IDF feature of the evaluation data in the *Test_Feature_Data* folder. <br>
...
- Determines the index of the category(0~8) with the highest probability for each row in the *output.txt* file. <br>
- Compute Micro-F1, Macro-F1 scores by comparing the category index information of the document predicted by the model with the actual correct category shown in *answer.txt*.<br>
